...
function critter_p_proc()
    ...
    if fallout.local_var(5) == 1 then
        local self_obj = fallout.self_obj()
        if fallout.tile_num(self_obj) ~= 17145 then
            fallout.animate_move_obj_to_tile(self_obj, 17145, 0)
        end
    end
end
...
function talk_p_proc()
    ...
    if fallout.local_var(4) == 1 then
        fallout.float_msg(self_obj, fallout.message_str(651, fallout.random(134, 136)), 2)
    else
        fallout.set_local_var(4, 1)
        fallout.start_gdialog(651, self_obj, 4, -1, -1)
        fallout.gsay_start()
        Brother00()
        fallout.gsay_end()
