...
function Display_Armed_And_Locked()
    ...
    fallout.display_msg(fallout.message_str(832, DISPLAY_ARMED_AND_LOCKED_LOOKUP[index]))
end

function Display_Locked()
    local index = (Per << PE_SHIFT) | (Locks << LP_SHIFT)
    fallout.display_msg(fallout.message_str(832, DISPLAY_LOCKED_LOOKUP[index]))
end

function Display_Trapped()
    local index = (Per << PE_SHIFT) | (Traps << TR_SHIFT)
    fallout.display_msg(fallout.message_str(832, DISPLAY_TRAPPED_LOOKUP[index]))
end
