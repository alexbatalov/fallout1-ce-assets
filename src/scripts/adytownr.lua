...
function critter_p_proc()
    ...
    local self_obj = fallout.self_obj()
    if time.game_time_in_seconds() - fallout.global_var(340) >= 10
        and fallout.tile_distance_objs(self_obj, dude_obj) <= 4
        and fallout.global_var(251) == 0 then
        if fallout.global_var(613) == 2 then
            fallout.float_msg(self_obj, fallout.message_str(251, fallout.random(113, 119)), 0)
        else
            fallout.float_msg(self_obj, fallout.message_str(251, fallout.random(101, 112)), 0)
        end
        fallout.set_global_var(340, time.game_time_in_seconds())
    end
