...
function use_p_proc()
    fallout.script_overrides()
    if fallout.obj_pid(fallout.self_obj()) == 33555334 then
        fallout.move_to(fallout.dude_obj(), 17108, 2)
    else
        fallout.display_msg(fallout.message_str(786, 104))
    end
end
...
function use_obj_on_p_proc()
    ...
    if fallout.obj_pid(fallout.obj_being_used_with()) == 127 then
        ...
        if fallout.obj_pid(fallout.self_obj()) ~= 33555334 then
            ...
            v0 = fallout.create_object_sid(33555334, fallout.tile_num(fallout.self_obj()), fallout.elevation(fallout.self_obj()), 201)
