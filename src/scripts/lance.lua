...
local known = false
local go_to_Shady = false
local go_to_Raiders = false
...
function critter_p_proc()
    if hostile then
        ...
        if known then
            ...
            if self_tile_num < 20000 then
                local dest = fallout.tile_num_in_direction(self_tile_num, 0, 3)
...
function talk_p_proc()
    ...
    if fallout.global_var(12) ~= 0 then
        ...
        if known then
            ...
            if fallout.global_var(246) ~= 0 or fallout.global_var(155) < -30 or reputation.has_rep_berserker() or fallout.global_var(158) > 2 then
...
function Lance00a()
    ...
    if fallout.get_critter_stat(dude_obj, 0) > 7
        or fallout.obj_is_carrying_obj_pid(dude_obj, 15) ~= 0
        or fallout.obj_is_carrying_obj_pid(dude_obj, 118) ~= 0
        or fallout.obj_is_carrying_obj_pid(dude_obj, 13) ~= 0
        or fallout.obj_is_carrying_obj_pid(dude_obj, 12) ~= 0 then
        Lance08()
    else
        Lance09()
    end
...
function Lance06()
    fallout.gsay_reply(699,
        fallout.message_str(699, 123) ..
        fallout.proto_data(fallout.obj_pid(fallout.dude_obj()), 1) .. fallout.message_str(699, 124))
...
function Lance07()
    ...
    fallout.gsay_reply(699,
        fallout.message_str(699, 123) ..
        fallout.proto_data(fallout.obj_pid(fallout.dude_obj()), 1) .. fallout.message_str(699, 130))
