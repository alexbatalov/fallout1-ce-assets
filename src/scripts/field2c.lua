...
function map_enter_p_proc()
    fallout.set_external_var("Field2c_Ptr", fallout.self_obj())
    ...
    if (fallout.global_var(262) == 0) or (fallout.map_var(16) == 0) then
...
function map_update_p_proc()
    ...
    if fallout.global_var(262) == 0 then
        if (fallout.map_var(16) == 0) and (fallout.global_var(146) == 1) then
            fallout.set_map_var(16, 1)
            fallout.set_obj_visibility(fallout.self_obj(), 1)
        else
            fallout.set_obj_visibility(fallout.self_obj(), 1)
