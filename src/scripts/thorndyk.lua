...
local PlayerYellsOuch = false
local DoctorPostTreatmentResponse = 0
local HealPlayer = false
...
function Thorn02()
    local current_hp = fallout.get_critter_stat(fallout.dude_obj(), 35)
    local max_hp = fallout.get_critter_stat(fallout.dude_obj(), 7)
    ...
    elseif current_hp < max_hp and time.game_time_in_hours() - fallout.local_var(6) < 24 then
        if fallout.get_critter_stat(fallout.dude_obj(), 34) == 0 then
    ...
    elseif current_hp > max_hp // 2 and time.game_time_in_hours() - fallout.local_var(6) > 24 then
        if fallout.get_critter_stat(fallout.dude_obj(), 34) == 0 then
