    ...
    if fallout.local_var(0) == 0 then
        if pid == 75 then
            ...
            if fallout.is_success(fallout.roll_vs_skill(fallout.dude_obj(), 13, 0)) then
                if fallout.map_var(20) == 1 then
                    fallout.set_obj_visibility(fallout.external_var("field5a_Ptr"), true)
                    fallout.display_msg(fallout.message_str(875, 103))
                    fallout.give_exp_points(50)
                    fallout.set_map_var(20, 0)
                else
                    fallout.set_obj_visibility(fallout.external_var("field5a_Ptr"), false)
...
function damage_p_proc()
    if fallout.is_success(fallout.roll_vs_skill(fallout.dude_obj(), 11, 0)) then
        ...
        fallout.destroy_object(fallout.external_var("field5a_Ptr"))
