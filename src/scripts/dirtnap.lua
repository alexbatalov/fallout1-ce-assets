...
function critter_p_proc()
    ...
    if fallout.global_var(30) ~= 0 and fallout.global_var(31) ~= 2 then
        local self_obj = fallout.self_obj()
        if fallout.obj_can_see_obj(self_obj, dude_obj) and fallout.tile_distance_objs(self_obj, dude_obj) < 9 then
            fallout.float_msg(self_obj, fallout.message_str(791, 102), 2)
            hostile = true
        end
    end
    if hostile then
