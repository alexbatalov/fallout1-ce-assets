...
function start()
    if not(g0) then
        ...
        set_alert_tile()
        if fallout.global_var(146) and fallout.external_var("radio_room_attacked") then
            fallout.move_to(fallout.self_obj(), g4, 0)
        end
        g0 = 1
    else
...
function combat_p_proc()
    if not(fallout.obj_can_see_obj(fallout.self_obj(), fallout.dude_obj())) then
        fallout.critter_stop_attacking(fallout.self_obj())
    end
...
function set_alert_tile()
    ...
    if fallout.local_var(4) == 23328 then
        g4 = 25870
    else
        if fallout.local_var(4) == 22325 then
            g4 = 25468
        else
            if fallout.local_var(4) == 22329 then
                g4 = 25070
            end
        end
    end
...
function Goodbyes()
    g5 = fallout.message_str(634, fallout.random(100, 105))
end
