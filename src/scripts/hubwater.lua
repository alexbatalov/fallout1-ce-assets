...
fallout.create_external_var("local Thorndyke_Ptr")
...
function start()
    if fallout.script_action() == 15 then
        fallout.set_global_var(580, 1)
        if (fallout.game_time() // (10 * 60 * 60 * 24)) >= fallout.global_var(150) then
            fallout.set_global_var(14, 1)
        end
        Lighting()
        add_party()
        fallout.override_map_start(74, 60, 0, 3)
    else
