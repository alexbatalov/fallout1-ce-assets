...
function use_skill_on_p_proc()
    ...
    local self_is_locked = fallout.obj_is_locked(self_obj)
    if fallout.local_var(2) == 0 and self_is_locked then
        Trapped_And_Locked()
    elseif fallout.local_var(2) == 0 then
        Trapped_Door()
    elseif self_is_locked then
        Locked_Door()
    end
end
...
function use_obj_on_p_proc()
    ...
    if pid == 77 then
        ...
        if fallout.local_var(2) == 0 then
            local roll = fallout.roll_vs_skill(fallout.source_obj(), 11, -20)
            if fallout.is_success(roll) then
                -- FIXME: Using message id as message.
...
function Display_Armed_And_Locked()
    ...
    fallout.display_msg(fallout.message_str(306, DISPLAY_ARMED_AND_LOCKED_LOOKUP[index]))
end

function Display_Locked()
    local index = (Per << PE_SHIFT) | (Locks << LP_SHIFT)
    fallout.display_msg(fallout.message_str(306, DISPLAY_LOCKED_LOOKUP[index]))
end

function Display_Trapped()
    local index = (Per << PE_SHIFT) | (Traps << TR_SHIFT)
    fallout.display_msg(fallout.message_str(306, DISPLAY_TRAPPED_LOOKUP[index]))
end
